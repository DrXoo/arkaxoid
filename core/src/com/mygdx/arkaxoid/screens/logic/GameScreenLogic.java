...
public class GameScreenLogic implements ContactListener {
	final Arkaxoid game;
	...
	public Room pauseButton;

	public Array<PowerUp> powerUps;

	public GameScreenLogic(Arkaxoid game, Room room) {
		this.room = room;
		this.game = game;
		...
		// Load the ball -------------------
		// Initial Position
		initialX = pj.getX() + pj.getWidth() / 2;
		initialY = pj.getY() + pj.getHeight() + game.assets.texture_ball.getHeight();
		Ball newBall = BallManager.createBall(initialX, initialY, game.assets.texture_ball, world,
		...
		pauseButton = new Room("pauseButton", game.assets.pauseButton, Arkaxoid.WIDTH_RES - 50,
				Arkaxoid.HEIGHT_RES - 125, 50, 50);
	...
	private void controlThePauseButton() {
		// We wet the position of that touch/click
		touchPos.set(Gdx.input.getX(), Gdx.input.getY(), 0);
		// Transforms it to our camera settings
		game.camera.unproject(touchPos);

		if (pauseButton.getRectangle().contains(touchPos.x, touchPos.y) && Gdx.input.justTouched()) {
			if (!game.getState().equals(GameStateType.PAUSED)) {
				game.setState(GameStateType.PAUSED);
				this.gameTimeLeft.stop();
				pauseButton.setCurrentFrame(1);
			} else {
				game.setState(GameStateType.STARTED);
				this.gameTimeLeft.start();
				pauseButton.setCurrentFrame(0);
			}
		}
	}
	...
	private void wonGame() {
		...
		gameTimeLeft.stop();
		// Save the score
		int totalScore = score + gameTimeLeft.showTimeIsSeconds() * 10 + pj.getLives() * 10;
		if (totalScore > Integer.parseInt(game.progress.getScoreFromRoom(room.getName())))
			game.progress.setScoreToRoom(room.getName(), totalScore + "");
	}

	private boolean isTheGameFinish() {
		boolean result = true;
		if (blocks.size > 0)
			for (Block b : blocks) {
				if (!b.getType().equals(BlockType.INDESTRUCTIBLE)) {
					result = false;
					break;
				}
			}
	...
	private void resetGame() {
		...
		// If the number of lives is 0, the player lost the game
		if (pj.getLives() == 0) {
		...
		// Set the startedTime with the right now time
		timeToStart = new Timer(0, 3, 1);
		timeToStart.start();
	}
	...
	private void updateBalls() {
		for (Ball ball : balls) {
			...
			if (ball.getPhysicalY() < 0) {
				if (balls.size == 1) {
					resetGame();
					// And stop the movement of the ball
					ball.setInMovement(false);
					// The position of the ball has to be the initial position
					ball.resetPosition();
				} else {
